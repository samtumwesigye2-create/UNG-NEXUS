...
def triage(entity_id,legacy,modern,field=None):
    ...
    if tier==1:
        _locked[entity_id]=event;_holds[entity_id]={"legacy":legacy,"modern":modern,"event":event,"state":"manual-review"}; put_divergence_hold(entity_id,tier,legacy,modern,field,"manual-review") if db_configured() else None
