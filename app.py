...
app=FastAPI(title='UNG-NEXUS',version='0.3.0')
...
def init():
    if DB:
        with conn() as c:
            ...
            c.execute('ALTER TABLE nexus_messages ADD COLUMN IF NOT EXISTS attempts INTEGER NOT NULL DEFAULT 0')
            c.execute('ALTER TABLE nexus_messages ADD COLUMN IF NOT EXISTS response_code INTEGER')
            c.execute('ALTER TABLE nexus_messages ADD COLUMN IF NOT EXISTS delivery_error TEXT')
            c.execute('ALTER TABLE nexus_messages ADD COLUMN IF NOT EXISTS delivered_at TIMESTAMPTZ')
...
@app.get('/health')
def health(): return {'status':'ok','service':'UNG-NEXUS','version':'0.3.0'}
...
@app.get('/v1/system')
def system():
    return {'system_id':'UNG-NEXUS','domain':'integration-interoperability','capabilities':['endpoint-registry','message-routing','outbound-http-delivery','retry','integration-audit','janus-bearer-auth']}
...
def deliver(url:str, envelope:dict, authorization:str):
    body=json.dumps(envelope,separators=(',',':')).encode()
    last_error=None
    last_code=None
    for attempt in range(1,DELIVERY_RETRIES+1):
        req=urllib.request.Request(url,data=body,method='POST',headers={'Content-Type':'application/json','Authorization':authorization,'User-Agent':'UNG-NEXUS/0.3'})
        try:
            with urllib.request.urlopen(req,timeout=DELIVERY_TIMEOUT) as r:
                code=int(r.status)
                if 200 <= code < 300:
                    return True,attempt,code,None
                last_code=code
...
def route_message(b:MessageIn,authorization:str|None=Header(None)):
    ...
    with conn() as c:
        ...
        if not target:
            return c.execute('INSERT INTO nexus_messages(id,source_system,target_system,message_type,payload,status,created_at,attempts,delivery_error) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *',(mid,b.source_system,b.target_system,b.message_type,psycopg.types.json.Jsonb(b.payload),'unroutable',created,0,'no_enabled_endpoint')).fetchone()
    ...
    ok,attempts,code,error=deliver(target['base_url'],envelope,authorization)
    status='delivered' if ok else 'failed'
    delivered_at=utcnow() if ok else None
    with conn() as c:
        return c.execute('INSERT INTO nexus_messages(id,source_system,target_system,message_type,payload,status,created_at,attempts,response_code,delivery_error,delivered_at) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *',(mid,b.source_system,b.target_system,b.message_type,psycopg.types.json.Jsonb(b.payload),status,created,attempts,code,error,delivered_at)).fetchone()
