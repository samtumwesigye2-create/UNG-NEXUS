...
app=FastAPI(title='UNG-NEXUS',version='0.4.4')
...
def auth(permission, authorization):
    ...
    req=urllib.request.Request(JANUS_BASE_URL+'/v1/auth/introspect',data=b'',method='POST',headers={'Authorization':authorization,'User-Agent':'UNG-NEXUS/0.4.4'})
    try:
...
def run_apollo_acceptance_probe():
    ...
    now=utcnow(); status='failed'; response_code=None; message_id=None; error=None
    try:
        req=urllib.request.Request(APOLLO_BASE_URL+'/v1/nexus/acceptance',data=b'',method='POST',headers={'User-Agent':'UNG-NEXUS/0.4.4'})
        with urllib.request.urlopen(req,timeout=DELIVERY_TIMEOUT) as r:
            response_code=int(r.status); body=json.loads(r.read().decode() or '{}')
            message_id=body.get('message_id'); status='passed' if 200<=response_code<300 and body.get('accepted') else 'failed'
    except urllib.error.HTTPError as e:
        response_code=int(e.code); error=f'http_{e.code}'
    except Exception as e:error=type(e).__name__
    try:
        with conn() as c:
            c.execute('''INSERT INTO nexus_acceptance_checks(id,target_system,status,response_code,message_id,error,created_at)
                         VALUES(%s,%s,%s,%s,%s,%s,%s)''',(str(uuid4()),'UNG-APOLLO',status,response_code,message_id,error,now))
...
def init():
    if DB:
        with conn() as c:
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS nexus_acceptance_checks(
                         id UUID PRIMARY KEY,target_system TEXT,status TEXT,response_code INTEGER,
                         message_id TEXT,error TEXT,created_at TIMESTAMPTZ)''')
...
@app.get('/')
def root():
    return {'service':'UNG-NEXUS','name':'Uganda National Grid Integration & Interoperability Platform','status':'online','version':'0.4.4','health':'/health','readiness':'/ready','system':'/v1/system','acceptance':'/v1/acceptance/status','docs':'/docs'}

@app.get('/health')
def health(): return {'status':'ok','service':'UNG-NEXUS','version':'0.4.4'}
@app.get('/ready')
def ready():
    try:
        with conn() as c:
            c.execute('SELECT 1')
            probe=c.execute("SELECT status,response_code,message_id,error,created_at FROM nexus_acceptance_checks WHERE target_system='UNG-APOLLO' ORDER BY created_at DESC LIMIT 1").fetchone()
...
@app.get('/v1/system')
def system(): return {'system_id':'UNG-NEXUS','domain':'integration-interoperability','capabilities':['endpoint-registry','message-routing','inbound-gateway','outbound-http-delivery','retry','integration-audit','janus-bearer-auth','apollo-route','apollo-acceptance-probe']}
@app.get('/v1/acceptance/status')
def acceptance_status():
    try:
        with conn() as c:
            row=c.execute("SELECT target_system,status,response_code,message_id,error,created_at FROM nexus_acceptance_checks WHERE target_system='UNG-APOLLO' ORDER BY created_at DESC LIMIT 1").fetchone()
...
def deliver(url,envelope,authorization):
    ...
    for attempt in range(1,DELIVERY_RETRIES+1):
        req=urllib.request.Request(url,data=body,method='POST',headers={'Content-Type':'application/json','Authorization':authorization,'User-Agent':'UNG-NEXUS/0.4.4'})
        try:
            with urllib.request.urlopen(req,timeout=DELIVERY_TIMEOUT) as r:
                code=int(r.status)
